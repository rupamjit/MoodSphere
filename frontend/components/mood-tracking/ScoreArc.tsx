...
export function ScoreArc({ score, size = 140 }: { score: number; size?: number }) {
  ...
  const isUnitScale = raw >= -1 && raw <= 1
  const normalized = isUnitScale
    ? Math.max(0, Math.min(100, (raw + 1) * 50))
    : Math.max(0, Math.min(100, raw))
  const displayScore = isUnitScale ? raw.toFixed(2) : String(Math.round(raw))

  const strokeW = 6, r = (size - strokeW * 2) / 2
  const cx = size / 2, cy = size / 2
  const startAngle = -220, totalArc = 260
  const endAngle = startAngle + (normalized / 100) * totalArc
  ...
  const color = normalized >= 68 ? "#10b981" : normalized >= 45 ? "#f59e0b" : "#ef4444"
  ...
  return (
    <svg width={size} height={size}>
      ...
      <text x={cx} y={cy - 2} textAnchor="middle"
        style={{ fontFamily: "'Georgia','Times New Roman',serif", fontSize: 30, fontWeight: 800, fill: "#111827" }}>{displayScore}</text>
