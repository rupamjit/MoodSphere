...
import { Mic, MicOff, Camera, CameraOff, Square, Brain, Loader2, Radio, Circle } from "lucide-react"
...
export function SessionView({
  ...
  // Attach the camera stream when the video element mounts
  useEffect(() => {
    let active = true
  ...
  return (
    <div className="w-full h-full bg-background p-4 md:p-6" style={{ height: "calc(100vh - 64px)" }}>
      <div className="grid h-full gap-4 md:grid-cols-[1.2fr_1fr]">
        <Card className="h-full rounded-2xl border-border/60 bg-card/95 py-0">
          ...
          <CardContent className="flex h-full flex-col gap-4 px-4 pb-4 pt-4 md:px-5">
            <div className="relative flex-1 overflow-hidden rounded-xl border bg-muted/40">
              ...
              <AnimatePresence>
                {(!hasVideo || !camActive) && (
                  ...
                  >
                    {!hasVideo ? <Brain className="size-8 text-muted-foreground" /> : <CameraOff className="size-8 text-muted-foreground" />}
                    <p className="text-sm text-muted-foreground">
                      {!hasVideo ? "Preparing camera and microphone" : "Camera is paused"}
                    </p>
                  </motion.div>
                )}
              </AnimatePresence>
            ...
            <div className="rounded-xl border bg-muted/20 p-3">
              <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Current AI Question</p>
            ...
            <div className="grid gap-3 md:grid-cols-[1fr_auto]">
              <Textarea
                ...
                placeholder="Optional transcript text for this answer..."
              ...
              <div className="flex flex-row gap-2 md:flex-col">
                ...
                <Button variant="outline" onClick={onToggleCam}>
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 border-t pt-3">
              ...
              <Button variant="destructive" onClick={onStopRecording} disabled={!isRecording}>
        ...
        <Card className="h-full rounded-2xl border-border/60 bg-card/95 py-0">
          <CardHeader className="border-b px-4 py-4 md:px-5">
            ...
            <CardDescription>After each stop, the next question starts automatically.</CardDescription>
          </CardHeader>
          <CardContent className="flex h-full flex-col px-4 pb-4 pt-4 md:px-5">
